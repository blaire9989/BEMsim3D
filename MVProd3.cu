/* 4-GPU implementation for computing matrix-vector products in BEM, for dielectric surface materials.
Users do not need to read or understand any method in this file. 
PLEASE DO NOT MODIFY THE FOLLOWING CODE. */

#include "MVProd3.h"

/// @brief 4-GPU implementation for computing matrix-vector products in BEM, for dielectric surface materials
...
MVProd3::MVProd3(Estimate* est, Singular* singular, Grid* grid, int ind0, int ind1, int ind2, int ind3): MVProd(est, singular, grid) {
    alpha.x = 1.0f;
    alpha.y = 0.0f;
    beta.x = 1.0f;
    beta.y = 0.0f;
...
void MVProd3::initializeFar() {
    ...
    for (int dev = 0; dev < 2; dev++) {
        ...
        cudaMalloc(&g2_data[dev], N * sizeof(Tfcomp));
        cudaMalloc(&g3_data[dev], N * sizeof(Tfcomp));
    ...
    for (int dev = 2; dev < 4; dev++) {
        ...
        cudaMalloc(&g5_data[dev - 2], N * sizeof(Tfcomp));
        cudaMalloc(&g6_data[dev - 2], N * sizeof(Tfcomp));
...
void MVProd3::setParameters(double eta1, dcomp eta2, double lambda) {
    ...
    for (int dev = 1; dev <= 2; dev++) {
        cudaSetDevice(devNumber[dev]);
        clearData<<< nnzB[dev] / 256 + 1, 256 >>>(d_Bee[dev], nnzB[dev]);
        clearData<<< nnzB[dev] / 256 + 1, 256 >>>(d_Bem[dev], nnzB[dev]);
        clearData<<< nnzB[dev] / 256 + 1, 256 >>>(d_Bmm[dev], nnzB[dev]);
        individual<<< nnzB[dev] / 256 + 1, 256 >>>(nnzB[dev], Nx, Ny, zvals[dev], d, k1, k2, const1, const21, const22, xvech[dev], wvech[dev], xvecl[dev], wvecl[dev], d_Brows[dev], d_Bcols[dev], 0, 0, 0, 0, 2, -1, -1, d_Bee[dev], d_Bmm[dev], d_Bem[dev]);
        individual<<< nnzB[dev] / 256 + 1, 256 >>>(nnzB[dev], Nx, Ny, zvals[dev], d, k1, k2, const1, const21, const22, xvech[dev], wvech[dev], xvecl[dev], wvecl[dev], d_Brows[dev], d_Bcols[dev], 0, 0, 0, 1, 2, -1, 1, d_Bee[dev], d_Bmm[dev], d_Bem[dev]);
        individual<<< nnzB[dev] / 256 + 1, 256 >>>(nnzB[dev], Nx, Ny, zvals[dev], d, k1, k2, const1, const21, const22, xvech[dev], wvech[dev], xvecl[dev], wvecl[dev], d_Brows[dev], d_Bcols[dev], 1, 0, 0, 0, 2, 1, -1, d_Bee[dev], d_Bmm[dev], d_Bem[dev]);
        individual<<< nnzB[dev] / 256 + 1, 256 >>>(nnzB[dev], Nx, Ny, zvals[dev], d, k1, k2, const1, const21, const22, xvech[dev], wvech[dev], xvecl[dev], wvecl[dev], d_Brows[dev], d_Bcols[dev], 1, 0, 0, 1, 2, 1, 1, d_Bee[dev], d_Bmm[dev], d_Bem[dev]);
        correctionHV<<< nnzB[dev] / 256 + 1, 256 >>>(nnzB[dev], dx, dy, dz, k1, const1, const21, totalY, totalZ, d_Brows[dev], d_Bcols[dev], num_pts, d_hori_b[dev], d_vert_b[dev], d_hori_x[dev], d_hori_z[dev], d_hori_d[dev], d_vert_y[dev], d_vert_z[dev], d_vert_d[dev], d_Bee[dev], d_Bem[dev]);
    }
    ...
    cudaSetDevice(devNumber[0]);
    correctionHH<<< nnzB[0] / 256 + 1, 256 >>>(nnzB[0], dx, dy, dz, k2, const1, const22, totalY, totalZ, d_Brows[0], d_Bcols[0], num_pts, d_hori_b[0], d_hori_x[0], d_hori_z[0], d_hori_d[0], d_Bmm[0], d_Bem[0]);
    postProcess<<< nnzB[0] / 256 + 1, 256 >>>(nnzB[0], true, eta0FL, eta1, eta2, d_Brows[0], d_Bcols[0], d_Bee[0], d_Bmm[0], d_Bem[0]);
    for (int dev = 1; dev <= 2; dev++) {
        cudaSetDevice(devNumber[dev]);
        correctionHV<<< nnzB[dev] / 256 + 1, 256 >>>(nnzB[dev], dx, dy, dz, k2, const1, const22, totalY, totalZ, d_Brows[dev], d_Bcols[dev], num_pts, d_hori_b[dev], d_vert_b[dev], d_hori_x[dev], d_hori_z[dev], d_hori_d[dev], d_vert_y[dev], d_vert_z[dev], d_vert_d[dev], d_Bmm[dev], d_Bem[dev]);
        postProcess<<< nnzB[dev] / 256 + 1, 256 >>>(nnzB[dev], false, eta0FL, eta1, eta2, d_Brows[dev], d_Bcols[dev], d_Bee[dev], d_Bmm[dev], d_Bem[dev]);
    }
    cudaSetDevice(devNumber[3]);
    correctionVV<<< nnzB[3] / 256 + 1, 256 >>>(nnzB[3], dx, dy, dz, k2, const1, const22, totalY, totalZ, d_Brows[3], d_Bcols[3], num_pts, d_vert_b[3], d_vert_y[3], d_vert_z[3], d_vert_d[3], d_Bmm[3], d_Bem[3]);
    postProcess<<< nnzB[3] / 256 + 1, 256 >>>(nnzB[3], true, eta0FL, eta1, eta2, d_Brows[3], d_Bcols[3], d_Bee[3], d_Bmm[3], d_Bem[3]);
    cudaDeviceSynchronize();

    // Transfer different farfield matching coefficients to different GPUs
    for (int dev = 0; dev < 4; dev += 2) {
        cudaSetDevice(devNumber[dev]);
        cudaMemcpy(d_hori_x[dev], (fcomp*)(grid->hori_x.data()), num_pts * hori_num * sizeof(fcomp), cudaMemcpyHostToDevice);
        cudaMemcpy(d_hori_z[dev], (fcomp*)(grid->hori_z.data()), num_pts * hori_num * sizeof(fcomp), cudaMemcpyHostToDevice);
        cudaMemcpy(d_hori_d[dev], (fcomp*)(grid->hori_d.data()), num_pts * hori_num * sizeof(fcomp), cudaMemcpyHostToDevice);
    ...
    computeGreens<<< N / 256 + 1, 256 >>>(g2_data[0], dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 2);
    cufftExecC2C(plan[0], (cufftComplex*)g2_data[0], (cufftComplex*)g2_data[0], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g3_data[0], dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 3);
    cufftExecC2C(plan[0], (cufftComplex*)g3_data[0], (cufftComplex*)g3_data[0], CUFFT_FORWARD);
    cudaSetDevice(devNumber[1]);
    computeGreens<<< N / 256 + 1, 256 >>>(g0_data[1], dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 0);
    cufftExecC2C(plan[1], (cufftComplex*)g0_data[1], (cufftComplex*)g0_data[1], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g1_data[1], dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 1);
    cufftExecC2C(plan[1], (cufftComplex*)g1_data[1], (cufftComplex*)g1_data[1], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g2_data[1], dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 2);
    cufftExecC2C(plan[1], (cufftComplex*)g2_data[1], (cufftComplex*)g2_data[1], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g3_data[1], dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 3);
    cufftExecC2C(plan[1], (cufftComplex*)g3_data[1], (cufftComplex*)g3_data[1], CUFFT_FORWARD);
    cudaSetDevice(devNumber[2]);
    computeGreens<<< N / 256 + 1, 256 >>>(g4_data[0], dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 4);
    cufftExecC2C(plan[2], (cufftComplex*)g4_data[0], (cufftComplex*)g4_data[0], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g5_data[0], dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 5);
    cufftExecC2C(plan[2], (cufftComplex*)g5_data[0], (cufftComplex*)g5_data[0], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g6_data[0], dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 6);
    cufftExecC2C(plan[2], (cufftComplex*)g6_data[0], (cufftComplex*)g6_data[0], CUFFT_FORWARD);
    cudaSetDevice(devNumber[3]);
    computeGreens<<< N / 256 + 1, 256 >>>(g4_data[1], dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 4);
    cufftExecC2C(plan[3], (cufftComplex*)g4_data[1], (cufftComplex*)g4_data[1], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g5_data[1], dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 5);
    cufftExecC2C(plan[3], (cufftComplex*)g5_data[1], (cufftComplex*)g5_data[1], CUFFT_FORWARD);
    computeGreens<<< N / 256 + 1, 256 >>>(g6_data[1], dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 6);
    cufftExecC2C(plan[3], (cufftComplex*)g6_data[1], (cufftComplex*)g6_data[1], CUFFT_FORWARD);
    cudaDeviceSynchronize();
}
...
VectorXcf MVProd3::multiply(VectorXcf x) {
...
void MVProd3::gpu0() {
    ...
    // Far matrix multiplication for block (Medium1, Zee): FFT pair 1
    ...
    // Far matrix multiplication for block (Medium1, Zee): FFT pair 2
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[0], N);
    ...
    // Far matrix multiplication for block (Medium1, Zee): FFT pair 3
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[0], N);
    ...
    // Far matrix multiplication for block (Medium1, Zee): FFT pair 4
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[0], N);
    ...
    // Far matrix multiplication for block (Medium1, Zmm): FFT pair 1
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[0], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo0_data[0], d_x3[0], hori_row, hori_col, num_pts, d_hori_x[0], d_hori_f[0]);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g2_data[0], geo0_data[0], N);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3[0], geo0_data[0], hori_num, num_pts, N, d_hori_x[0], d_hori_b[0], true);
    
    // Far matrix multiplication for block (Medium1, Zmm): FFT pair 2
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[0], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo0_data[0], d_x4[0], vert_row, vert_col, num_pts, d_vert_y[0], d_vert_f[0]);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g2_data[0], geo0_data[0], N);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4[0], geo0_data[0], vert_num, num_pts, N, d_vert_y[0], d_vert_b[0], true);

    // Far matrix multiplication for block (Medium1, Zmm): FFT pair 3
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[0], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo0_data[0], d_x3[0], hori_row, hori_col, num_pts, d_hori_z[0], d_hori_f[0]);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo0_data[0], d_x4[0], vert_row, vert_col, num_pts, d_vert_z[0], d_vert_f[0]);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g2_data[0], geo0_data[0], N);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3[0], geo0_data[0], hori_num, num_pts, N, d_hori_z[0], d_hori_b[0], true);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4[0], geo0_data[0], vert_num, num_pts, N, d_vert_z[0], d_vert_b[0], true);

    // Far matrix multiplication for block (Medium1, Zmm): FFT pair 4
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[0], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo0_data[0], d_x3[0], hori_row, hori_col, num_pts, d_hori_d[0], d_hori_f[0]);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo0_data[0], d_x4[0], vert_row, vert_col, num_pts, d_vert_d[0], d_vert_f[0]);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g3_data[0], geo0_data[0], N);
    cufftExecC2C(plan[0], (cufftComplex*)geo0_data[0], (cufftComplex*)geo0_data[0], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3[0], geo0_data[0], hori_num, num_pts, N, d_hori_d[0], d_hori_b[0], false);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4[0], geo0_data[0], vert_num, num_pts, N, d_vert_d[0], d_vert_b[0], false);
}
...
void MVProd3::gpu1() {
    ...
    // Far matrix multiplication for block (Medium2, Zee): FFT pair 1
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo0_data[1], d_x1[1], hori_row, hori_col, num_pts, d_hori_x[1], d_hori_f[1]);
    ...
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1[1], geo0_data[1], hori_num, num_pts, N, d_hori_x[1], d_hori_b[1], false);
    
    // Far matrix multiplication for block (Medium2, Zee): FFT pair 2
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo0_data[1], d_x2[1], vert_row, vert_col, num_pts, d_vert_y[1], d_vert_f[1]);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g0_data[1], geo0_data[1], N);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2[1], geo0_data[1], vert_num, num_pts, N, d_vert_y[1], d_vert_b[1], false);
    
    // Far matrix multiplication for block (Medium2, Zee): FFT pair 3
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo0_data[1], d_x1[1], hori_row, hori_col, num_pts, d_hori_z[1], d_hori_f[1]);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo0_data[1], d_x2[1], vert_row, vert_col, num_pts, d_vert_z[1], d_vert_f[1]);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g0_data[1], geo0_data[1], N);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1[1], geo0_data[1], hori_num, num_pts, N, d_hori_z[1], d_hori_b[1], false);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2[1], geo0_data[1], vert_num, num_pts, N, d_vert_z[1], d_vert_b[1], false);

    // Far matrix multiplication for block (Medium2, Zee): FFT pair 4
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo0_data[1], d_x1[1], hori_row, hori_col, num_pts, d_hori_d[1], d_hori_f[1]);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo0_data[1], d_x2[1], vert_row, vert_col, num_pts, d_vert_d[1], d_vert_f[1]);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g1_data[1], geo0_data[1], N);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1[1], geo0_data[1], hori_num, num_pts, N, d_hori_d[1], d_hori_b[1], true);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2[1], geo0_data[1], vert_num, num_pts, N, d_vert_d[1], d_vert_b[1], true);

    // Far matrix multiplication for block (Medium2, Zmm): FFT pair 1
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo0_data[1], d_x3[1], hori_row, hori_col, num_pts, d_hori_x[1], d_hori_f[1]);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g2_data[1], geo0_data[1], N);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3[1], geo0_data[1], hori_num, num_pts, N, d_hori_x[1], d_hori_b[1], true);
    
    // Far matrix multiplication for block (Medium2, Zmm): FFT pair 2
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    ...
    // Far matrix multiplication for block (Medium2, Zmm): FFT pair 3
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    ...
    // Far matrix multiplication for block (Medium2, Zmm): FFT pair 4
    clearData<<< N / 256 + 1, 256 >>>(geo0_data[1], N);
    ...
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(g3_data[1], geo0_data[1], N);
    cufftExecC2C(plan[1], (cufftComplex*)geo0_data[1], (cufftComplex*)geo0_data[1], CUFFT_INVERSE);
...
void MVProd3::gpu2() {
    ...
    // Far matrix multiplication for block (Medium1, Zem): FFT group 1
    ...
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g6_data[0], geo2_data[0], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[0], N);
    ...
    cufftExecC2C(plan[2], (cufftComplex*)geo2_data[0], (cufftComplex*)geo2_data[0], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g5_data[0], geo2_data[0], N, true);
    ...
    // Far matrix multiplication for block (Medium1, Zem): FFT group 2
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[0], N);
    ...
    cufftExecC2C(plan[2], (cufftComplex*)geo2_data[0], (cufftComplex*)geo2_data[0], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g6_data[0], geo2_data[0], N, true);
    cufftExecC2C(plan[2], (cufftComplex*)geo1_data[0], (cufftComplex*)geo1_data[0], CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2[2], geo1_data[0], vert_num, num_pts, N, d_vert_y[2], d_vert_b[2], false);

    // Far matrix multiplication for block (Medium1, Zem): FFT group 3
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[0], N);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g5_data[0], geo2_data[0], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[0], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[0], d_x4[2], vert_row, vert_col, num_pts, d_vert_y[2], d_vert_f[2]);
    cufftExecC2C(plan[2], (cufftComplex*)geo2_data[0], (cufftComplex*)geo2_data[0], CUFFT_FORWARD);
    ...
    // Far matrix multiplication for block (Medium1, Zme): FFT group 1
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[0], N);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[0], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[0], d_x2[2], vert_row, vert_col, num_pts, d_vert_y[2], d_vert_f[2]);
    cufftExecC2C(plan[2], (cufftComplex*)geo2_data[0], (cufftComplex*)geo2_data[0], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g6_data[0], geo2_data[0], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[0], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo2_data[0], d_x1[2], hori_row, hori_col, num_pts, d_hori_z[2], d_hori_f[2]);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[0], d_x2[2], vert_row, vert_col, num_pts, d_vert_z[2], d_vert_f[2]);
    cufftExecC2C(plan[2], (cufftComplex*)geo2_data[0], (cufftComplex*)geo2_data[0], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g5_data[0], geo2_data[0], N, true);
    cufftExecC2C(plan[2], (cufftComplex*)geo1_data[0], (cufftComplex*)geo1_data[0], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3[2], geo1_data[0], hori_num, num_pts, N, d_hori_x[2], d_hori_b[2], false);

    // Far matrix multiplication for block (Medium1, Zme): FFT group 2
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[0], N);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g4_data[0], geo2_data[0], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[0], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo2_data[0], d_x1[2], hori_row, hori_col, num_pts, d_hori_x[2], d_hori_f[2]);
    cufftExecC2C(plan[2], (cufftComplex*)geo2_data[0], (cufftComplex*)geo2_data[0], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g6_data[0], geo2_data[0], N, true);
    cufftExecC2C(plan[2], (cufftComplex*)geo1_data[0], (cufftComplex*)geo1_data[0], CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4[2], geo1_data[0], vert_num, num_pts, N, d_vert_y[2], d_vert_b[2], false);

    // Far matrix multiplication for block (Medium1, Zme): FFT group 3
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[0], N);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g5_data[0], geo2_data[0], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[0], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[0], d_x2[2], vert_row, vert_col, num_pts, d_vert_y[2], d_vert_f[2]);
    ...
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[0], g4_data[0], geo2_data[0], N, true);
    cufftExecC2C(plan[2], (cufftComplex*)geo1_data[0], (cufftComplex*)geo1_data[0], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3[2], geo1_data[0], hori_num, num_pts, N, d_hori_z[2], d_hori_b[2], false);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4[2], geo1_data[0], vert_num, num_pts, N, d_vert_z[2], d_vert_b[2], false);
}
...
void MVProd3::gpu3() {
    ...
    // Far matrix multiplication for block (Medium2, Zem): FFT group 1
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[1], N);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[1], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[1], d_x4[3], vert_row, vert_col, num_pts, d_vert_y[3], d_vert_f[3]);
    cufftExecC2C(plan[3], (cufftComplex*)geo2_data[1], (cufftComplex*)geo2_data[1], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g6_data[1], geo2_data[1], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[1], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo2_data[1], d_x3[3], hori_row, hori_col, num_pts, d_hori_z[3], d_hori_f[3]);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[1], d_x4[3], vert_row, vert_col, num_pts, d_vert_z[3], d_vert_f[3]);
    cufftExecC2C(plan[3], (cufftComplex*)geo2_data[1], (cufftComplex*)geo2_data[1], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g5_data[1], geo2_data[1], N, true);
    cufftExecC2C(plan[3], (cufftComplex*)geo1_data[1], (cufftComplex*)geo1_data[1], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1[3], geo1_data[1], hori_num, num_pts, N, d_hori_x[3], d_hori_b[3], false);

    // Far matrix multiplication for block (Medium2, Zem): FFT group 2
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[1], N);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g4_data[1], geo2_data[1], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[1], N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geo2_data[1], d_x3[3], hori_row, hori_col, num_pts, d_hori_x[3], d_hori_f[3]);
    cufftExecC2C(plan[3], (cufftComplex*)geo2_data[1], (cufftComplex*)geo2_data[1], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g6_data[1], geo2_data[1], N, true);
    cufftExecC2C(plan[3], (cufftComplex*)geo1_data[1], (cufftComplex*)geo1_data[1], CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2[3], geo1_data[1], vert_num, num_pts, N, d_vert_y[3], d_vert_b[3], false);

    // Far matrix multiplication for block (Medium2, Zem): FFT group 3
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[1], N);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g5_data[1], geo2_data[1], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[1], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[1], d_x4[3], vert_row, vert_col, num_pts, d_vert_y[3], d_vert_f[3]);
    cufftExecC2C(plan[3], (cufftComplex*)geo2_data[1], (cufftComplex*)geo2_data[1], CUFFT_FORWARD);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g4_data[1], geo2_data[1], N, true);
    cufftExecC2C(plan[3], (cufftComplex*)geo1_data[1], (cufftComplex*)geo1_data[1], CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1[3], geo1_data[1], hori_num, num_pts, N, d_hori_z[3], d_hori_b[3], false);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2[3], geo1_data[1], vert_num, num_pts, N, d_vert_z[3], d_vert_b[3], false);

    // Far matrix multiplication for block (Medium2, Zme): FFT group 1
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[1], N);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[1], N);
    ...
    // Far matrix multiplication for block (Medium2, Zme): FFT group 2
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[1], N);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g4_data[1], geo2_data[1], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[1], N);
    ...
    // Far matrix multiplication for block (Medium2, Zme): FFT group 3
    clearData<<< N / 256 + 1, 256 >>>(geo1_data[1], N);
    convolveTransfer<<< N / 256 + 1, 256 >>>(geo1_data[1], g5_data[1], geo2_data[1], N, false);
    clearData<<< N / 256 + 1, 256 >>>(geo2_data[1], N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geo2_data[1], d_x2[3], vert_row, vert_col, num_pts, d_vert_y[3], d_vert_f[3]);
...
void MVProd3::cleanAll() {
    ...
    for (int count = 0; count < 2; count++) {
        ...
        cudaFree(g5_data[count]);
        cudaFree(g6_data[count]);
