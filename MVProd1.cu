/* Memory-saving, 1-GPU implementation for computing matrix-vector products in BEM.
Users do not need to read or understand any method in this file. 
PLEASE DO NOT MODIFY THE FOLLOWING CODE. */

#include "MVProd1.h"

/// @brief Memory-saving, 1-GPU implementation for computing matrix-vector products in BEM
...
MVProd1::MVProd1(Estimate* est, Singular* singular, Grid* grid, bool isDielectric, int ind): MVProd(est, singular, grid) {
...
void MVProd1::initializeFar() {
    ...
    cudaMalloc(&greens, N * sizeof(Tfcomp));
    cudaMalloc(&geometry, N * sizeof(Tfcomp));
    cufftCreate(&plan);
...
void MVProd1::setParameters(double eta1, dcomp eta2, double lambda) {
    ...
    k1 = (float)(2 * M_PI / lambda) * e1;
    k2 = (float)(2 * M_PI / lambda) * e2;
...
VectorXcf MVProd1::multiply(VectorXcf x) {
...
void MVProd1::far0() {
    // Far matrix multiplication: Green's function 0
    computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 7);
    cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
    
    // FFT pair 1
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geometry, d_x1, hori_row, hori_col, num_pts, d_hori_x, d_hori_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, const1);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1, geometry, hori_num, num_pts, N, d_hori_x, d_hori_b, false);

    // FFT pair 2
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x2, vert_row, vert_col, num_pts, d_vert_y, d_vert_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, const1);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2, geometry, vert_num, num_pts, N, d_vert_y, d_vert_b, false);

    // FFT pair 3
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geometry, d_x1, hori_row, hori_col, num_pts, d_hori_z, d_hori_f);
    scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x2, vert_row, vert_col, num_pts, d_vert_z, d_vert_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, const1);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1, geometry, hori_num, num_pts, N, d_hori_z, d_hori_b, false);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2, geometry, vert_num, num_pts, N, d_vert_z, d_vert_b, false);

    // FFT pair 4
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geometry, d_x1, hori_row, hori_col, num_pts, d_hori_d, d_hori_f);
    scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x2, vert_row, vert_col, num_pts, d_vert_d, d_vert_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, const21);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1, geometry, hori_num, num_pts, N, d_hori_d, d_hori_b, true);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2, geometry, vert_num, num_pts, N, d_vert_d, d_vert_b, true);

    // FFT pair 5
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geometry, d_x3, hori_row, hori_col, num_pts, d_hori_x, d_hori_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, e1 * e1 * const1);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_x, d_hori_b, true);

    // FFT pair 6
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x4, vert_row, vert_col, num_pts, d_vert_y, d_vert_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, e1 * e1 * const1);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4, geometry, vert_num, num_pts, N, d_vert_y, d_vert_b, true);

    // FFT pair 7
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geometry, d_x3, hori_row, hori_col, num_pts, d_hori_z, d_hori_f);
    scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x4, vert_row, vert_col, num_pts, d_vert_z, d_vert_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, e1 * e1 * const1);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_z, d_hori_b, true);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4, geometry, vert_num, num_pts, N, d_vert_z, d_vert_b, true);

    // FFT pair 8
    clearData<<< N / 256 + 1, 256 >>>(geometry, N);
    scatter<<< hori_col / 256 + 1, 256 >>>(geometry, d_x3, hori_row, hori_col, num_pts, d_hori_d, d_hori_f);
    scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x4, vert_row, vert_col, num_pts, d_vert_d, d_vert_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, e1 * e1 * const21);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_d, d_hori_b, false);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4, geometry, vert_num, num_pts, N, d_vert_d, d_vert_b, false);

    if (isDielectric) {
        computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 7);
        cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
        ...
        scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x2, vert_row, vert_col, num_pts, d_vert_D, d_vert_f);
        cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
        convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, const22);
        cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
        accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1, geometry, hori_num, num_pts, N, d_hori_D, d_hori_b, true);
        accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2, geometry, vert_num, num_pts, N, d_vert_D, d_vert_b, true);
        ...
        scatter<<< hori_col / 256 + 1, 256 >>>(geometry, d_x3, hori_row, hori_col, num_pts, d_hori_X, d_hori_f);
        cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
        convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, e2 * e2 * const1);
        cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
        accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_X, d_hori_b, true);
        ...
        scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x4, vert_row, vert_col, num_pts, d_vert_D, d_vert_f);
        cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
        convolveScale<<< N / 256 + 1, 256 >>>(greens, geometry, N, e2 * e2 * const22);
        cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
        accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_D, d_hori_b, false);
        accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4, geometry, vert_num, num_pts, N, d_vert_D, d_vert_b, false);
    }
...
void MVProd1::far1() {
    // Far matrix multiplication: Green's function 1
    computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 4);
    cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
    ...
    scatter<<< vert_col / 256 + 1, 256 >>>(geometry, d_x2, vert_row, vert_col, num_pts, d_vert_y, d_vert_f);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_FORWARD);
    convolve<<< N / 256 + 1, 256 >>>(greens, geometry, N);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_z, d_hori_b, true);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4, geometry, vert_num, num_pts, N, d_vert_z, d_vert_b, true);
    ...
    convolve<<< N / 256 + 1, 256 >>>(greens, geometry, N);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4, geometry, vert_num, num_pts, N, d_vert_y, d_vert_b, false);
    ...
    convolve<<< N / 256 + 1, 256 >>>(greens, geometry, N);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1, geometry, hori_num, num_pts, N, d_hori_z, d_hori_b, true);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2, geometry, vert_num, num_pts, N, d_vert_z, d_vert_b, true);
    ...
    if (isDielectric) {
        computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 4);
        cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
...
void MVProd1::far2() {
    // Far matrix multiplication: Green's function 2
    computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 5);
    cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
    ...
    convolve<<< N / 256 + 1, 256 >>>(greens, geometry, N);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_z, d_hori_b, false);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y4, geometry, vert_num, num_pts, N, d_vert_z, d_vert_b, false);
    ...
    convolve<<< N / 256 + 1, 256 >>>(greens, geometry, N);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y1, geometry, hori_num, num_pts, N, d_hori_x, d_hori_b, true);
    ...
    if (isDielectric) {
        computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 5);
        cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
...
void MVProd1::far3() {
    // Far matrix multiplication: Green's function 3
    computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e1, k1, const1, const21, N, totalX, totalY, totalZ, 6);
    cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
    ...
    convolve<<< N / 256 + 1, 256 >>>(greens, geometry, N);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< hori_num / 256 + 1, 256 >>>(d_y3, geometry, hori_num, num_pts, N, d_hori_x, d_hori_b, false);
    ...
    convolve<<< N / 256 + 1, 256 >>>(greens, geometry, N);
    cufftExecC2C(plan, (cufftComplex*)geometry, (cufftComplex*)geometry, CUFFT_INVERSE);
    accumulate<<< vert_num / 256 + 1, 256 >>>(d_y2, geometry, vert_num, num_pts, N, d_vert_y, d_vert_b, true);
    ...
    if (isDielectric) {
        computeGreens<<< N / 256 + 1, 256 >>>(greens, dx, dy, dz, eta0FL, e2, k2, const1, const22, N, totalX, totalY, totalZ, 6);
        cufftExecC2C(plan, (cufftComplex*)greens, (cufftComplex*)greens, CUFFT_FORWARD);
