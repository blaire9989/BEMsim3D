...
Solver::Solver(MatrixBuild* mat) {
    ...
    beta.y = 0.0f;
    cublasCreate(&handle);
    cudaMalloc(&d_x, N * sizeof(cuComplex));
    cudaMalloc(&d_y, N * sizeof(cuComplex));
    cudaMalloc(&d_Z, N * N * sizeof(cuComplex));
}

void Solver::transferMatrix(int polarization) {
    if (polarization == 1)
        cudaMemcpy(d_Z, (fcomp*)(mat->Z_TE.data()), N * N * sizeof(cuComplex), cudaMemcpyHostToDevice);
    else
        cudaMemcpy(d_Z, (fcomp*)(mat->Z_TM.data()), N * N * sizeof(cuComplex), cudaMemcpyHostToDevice);
}

VectorXcf Solver::multiply(VectorXcf h_x) {
    cudaMemcpy(d_x, (fcomp*)(h_x.data()), N * sizeof(cuComplex), cudaMemcpyHostToDevice);
    clearData<<< N / 256 + 1, 256 >>>(d_y, N);
    cublasCgemv(handle, CUBLAS_OP_N, N, N, &alpha, d_Z, N, d_x, 1, &beta, d_y, 1);
    VectorXcf h_y = VectorXcf::Zero(N);
    cudaMemcpy((fcomp*)(h_y.data()), d_y, N * sizeof(fcomp), cudaMemcpyDeviceToHost);
    return h_y;
}
...
VectorXcf Solver::symOrtho(fcomp a1, fcomp a2) {
    ...
    if (a1.imag() == 0 && a2.imag() == 0) {
        if (a2.real() == 0) {
            if (a1.real() == 0)
                results << 1.0f, 0.0f, absa1;
            else
                results << signa1, 0.0f, absa1;
            return results;
        } else if (a1.real() == 0) {
            results << 0.0, signa2, absa2;
            return results;
        }
        if (absa2 > absa1) {
            fcomp t = a1 / a2;
